%Spin Hall effect for momentum relaxation and phase relaxation
...
%constant
 ...
 qh = q/hbar;
zplus = 1i*1e-12;    %green function use small imaginary part
...
f2 =0;
 ...
 %%%%%%Sancho-ruby iteration solve surface green function 
    ...
    sig2in = gam2*f2;
       ...
       %green function self consistent for dephasing
        
     change = 100;
     while change > 1e-6

         G = inv((EE+zplus)*eye(2*NWp)-H-sig1-sig2-sigB);
%       sigBnew = DD*eye(2*NWp).*G; %momentum dephasing
        sigBnew = DD*G;         %phase dephasing
         change = sum(sum(abs(sigB-sigBnew)))/sum(sum(abs(sigB+sigBnew)));
%          change = sum(sum(abs(sigB-sigBnew)));
...
%         sigBinnew = DD*eye(2*NWp).*gn; %momentum dephasing
        sigBinnew = DD*gn; %phase dephasing
        change = sum(sum(abs(sigBin-sigBinnew)))/sum(sum(abs(sigBin+sigBinnew)))
%      change = sum(sum(abs(sigBin-sigBinnew)))
...
%   Gn = G*(gam1in+gam2in)*G';
 ...
 S0 = kron(eye(NWp),eye(2)); Sx = kron(eye(NWp),sx);
 Sy = kron(eye(NWp),sy);     Sz = kron(eye(NWp),sz);
...
for i =1:length(A00)
 ...
 Y = [0:1:NW-1];
%check mid-point chanel quasi-fermi-level 
%   f00 =reshape(f0,11,11);
%   f00 = f00(6,:);
%   
%   fxx =reshape(fx,11,11);
%   fxx = fxx(6,:);
%   
%   fyy =reshape(fy,11,11);
%   fyy = fyy(6,:);
%   
%   fzz =reshape(fz,11,11);
%   fzz = fzz(6,:);
%   
%   figure
%   plot(Y,f00);
...
%    aaa=(H(2*(i+NW)-1:2*(i+NW),2*i-1:2*i)*sz + sz*H(2*(i+NW)-1:2*(i+NW),2*i-1:2*i))*Gless(2*i-1:2*i,2*(i+NW)-1:2*(i+NW));
%    ccc =aaa';
%    bbb =(H(2*i-1:2*i,2*(i+NW)-1:2*(i+NW))*sz + sz*H(2*i-1:2*i,2*(i+NW)-1:2*(i+NW)))*Gless(2*(i+NW)-1:2*(i+NW),2*i-1:2*i);
end
